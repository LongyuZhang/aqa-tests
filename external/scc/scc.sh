...
getSemeruDockerfile() {
    if [[ ! -f "Dockerfile.open.releases.full" ]]; then
        if [[ $jdkVersion ]]; then
            ...
            # testtesttest https://raw.githubusercontent.com/LongyuZhang/semeru-containers/refs/heads/tmpp1/17/jdk/ubi/ubi9/Dockerfile.open.releases.full
            semeruDockerfileUrlBase="https://raw.githubusercontent.com/LongyuZhang/semeru-containers/tmpp1/$jdkVersionDir/jdk/${docker_os}"
...
prepareJMeterDockerfile() {
    echo "prepare JMeter at $(pwd)..."

    git clone https://github.com/mpirvu/dockerized-apps.git
    (
        cd dockerized-apps/DT8/JMeterContext
        # need to find a replacement to get this image
        findCommandAndReplace 'FROM adoptopenjdk\/openjdk8-openj9:jre' 'FROM docker.io\/adoptopenjdk\/openjdk8-openj9:jre' Dockerfile_jmeter true ";"
    )
...
buildImage() {
    ...
    # sudo podman build -t daytrader:latest -f Dockerfile.pingperf .
}

createRestoreImage() {
    echo "create restore image $restoreImage ..."
    sudo podman run --name ol-instanton-test-checkpoint-container --privileged --env WLP_CHECKPOINT=afterAppStart daytrader:latest
...
privilegedRestore() {
    ...
    jmeter -n -t daytrader8.jmx -j /output/daytrader.stats -JHOST=localhost -JPORT=9080 -JSTOCKS=9999 \
    -JBOTUID=0 -JTOPUID=14999 -JTHREADS=15 -JDURATION=60 -JRAMP=0 -JMAXTHINKTIME=0 >> containerId.log
