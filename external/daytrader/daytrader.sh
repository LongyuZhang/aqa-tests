...
prepare() {
    ...
    git clone git@github.ibm.com:rahil/semeru_container_test.git
    (
        cd semeru_container_test/DT8 || exit
...
buildImage() {
    ...
    sudo podman build --build-arg LIBERTY_IMAGE=icr.io/appcafe/open-liberty:beta-instanton  -t ${restoreImage}:latest  -f semeru_container_test/DT8/Dockerfile.daytrader8 semeru_container_test/DT8
...
getImageNameList() {
    ...
    if [[ $JOB_NAME == "Grinder" ]]; then
        ...
        comboList=DAYTRADER_COMBO_LIST_$platValue
