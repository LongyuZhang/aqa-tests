...
if [ $command_type == "prepare" ]; then
	# Specify docker.io or internal registry to prepare base image with login to increase pull limit or authenticate.
...
if [ $command_type == "build" ]; then
	echo "build_image.sh $test $version $impl $docker_os $package $build_type $platform $base_docker_registry_url $base_docker_registry_dir $check_external_custom $imageArg"
	source $(dirname "$0")/build_image.sh $test $version $impl $docker_os $package $build_type $platform "$base_docker_registry_url" "$base_docker_registry_dir" $check_external_custom $imageArg
fi
