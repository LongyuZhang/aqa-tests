...
if [ $command_type == "load" ]; then
	...
	docker image ls
	# restore
	echo "docker run --privileged $mountV --name restore-checkpoint --entrypoint '/bin/sh' $docker_registry_url/criu-restore-ready-with-jdk:${JDK_VERSION}-${JDK_IMPL}-${docker_os}-${platform}-${diff_micro_architecture} -c 'checkpoint_folders=\"/aqa-tests/TKG/output_*/cmdLineTester_criu_keepCheckpoint*\"; cd \$checkpoint_folders[0]; criu restore -D ./cpData --shell-job'"
