...
if [ $command_type == "load" ]; then
	...
	docker image ls
	# restore
	echo "docker run --privileged $mountV --name restore-checkpoint --entrypoint '/bin/sh' $docker_registry_url/criu-restore-ready-with-jdk:11-openj9-ubuntu-linux_x86-64-hw.arch.x86 -c 'cd /aqa-tests/TKG/output_*/cmdLineTester_criu_onlyCheckpoint*; criu restore -D ./cpData --shell-job'"
	docker run --privileged $mountV --name restore-checkpoint --entrypoint '/bin/sh' --rm $docker_registry_url/criu-restore-ready-with-jdk:11-openj9-ubuntu-linux_x86-64-hw.arch.x86 -c "cd /aqa-tests/TKG/output_*/cmdLineTester_criu_onlyCheckpoint*; criu restore -D ./cpData --shell-job; cat testOutput"
